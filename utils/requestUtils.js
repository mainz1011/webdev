 ...
 *
 * @param {http.incomingMessage} request incoming request message
...
const getCredentials = request => {
  ...
  const base64 = request.headers.authorization;
  if (!base64 || base64 === '' || !base64.includes('Basic')){
    return null;
  }
  const credentials = Buffer.from(base64.split(" ")[1], "base64").toString('ascii');
 ...
 *
 * @param {http.incomingMessage} request incoming request message
 * @returns {boolean} True if header includes application/json
 */
 ...
 *
 * @param {http.incomingMessage} request incoming request message
 * @returns {boolean} true if request content type is JSON
 */
