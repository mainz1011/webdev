 ...
 * @returns {boolean} true if email already in use
 */
 ...
 * @returns {object|undefined} user if match email and password
 */
 ...
 * @returns {object|undefined} user if match id
 */
...
const deleteUserById = userId => {
  ...
  // Hint: Array's findIndex() with user ID can could be used to find the user, 
  // and Array's splice() method can be used to "extract" the user object.
  const userIdx = data.users.findIndex((user) => user._id === userId);
  if (userIdx === -1){
    return undefined;
  }
...
const updateUserRole = (userId, role) => {
  // TODO: 8.3 Update user's role
    ...
    if (role === 'customer' ||role === 'admin'){
      const updatedUser = data.users.slice().find((user) => user._id === userId);
